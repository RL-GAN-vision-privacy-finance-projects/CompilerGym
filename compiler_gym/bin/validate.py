...
def main(argv):
    ...
    for result in validation_results:
        ...
        if result.failed:
            ...
            validated_rewards.append(result.state.reward)
            walltimes.append(result.state.walltime)

    # Print a summary footer.
    print("   |", "-" * name_col_width, "-|-------------|", sep="")
    print(
        f"    {'Mean inference walltime':<{name_col_width}}  "
        f"{arithmetic_mean(walltimes):9.4f} sec"
    )
    print(
        f"    {gmean_name:<{name_col_width}}  "
        f"{geometric_mean(validated_rewards):9.4f}"
    )
